...
if ( ! function_exists( 'prtfl_init_metaboxes' ) ) {
	function prtfl_init_metaboxes() {
		...
		add_meta_box( 'Page-Attributes', __( 'Page Attributes', 'page-attributes' ), 'page_attributes_meta_box', 'portfolio', 'side', 'high' );
	}
}
		 ...
		 if( 1 != $pages ) {
			 ...
			 for ( $i = 1; $i <= $pages; $i++ ) {
				 if ( 1 != $pages && ( !( $i >= $paged + $range + 1 || $i <= $paged - $range - 1 ) || $pages <= $showitems ) ) {
						 echo ( $paged == $i ) ? "<span class='current btn btn-mini btn-success'>". $i ."</span>":"<a href='". get_pagenum_link( $i) ."' class='inactive btn btn-mini btn-info' >". $i ."</a>";
				 }
			 }
